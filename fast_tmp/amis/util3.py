...
from fast_tmp.amis.schema.forms.widgets import ArrayItem, DatetimeItem, NumberItem, TextItem
...
def create_control(column, add_default_value) -> Control:
    ...
    if isinstance(column.type, SmallInteger):
        ...
        return NumberItem(
            min=column.info.get("min", -9223372036854775808),
            max=column.info.get("max", 9223372036854775808),
            precision=column.info.get("precision", 0),
            step=column.info.get("step", 1),
            **_get_base_attr(column),
            value=column.default if add_default_value and column.default else None,
            validations={
                "minimum": column.info.get("min", -9223372036854775808),
                "maximum": column.info.get("max", 9223372036854775808),
            },
        )
        ...
        return ArrayItem(
            items=column.info.get("items"),
            addable=column.info.get("addable"),
            removable=column.info.get("removable"),
            draggable=column.info.get("draggable"),
            draggableTip=column.info.get("draggableTip"),
            addButtonText=column.info.get("addButtonText"),
            minLength=column.info.get("minLength", 0),
            maxLength=column.info.get("maxLength"),
        )
        ...
        return DatetimeItem(
            value=column.info.get("value"),
            format=column.info.get("format"),
            inputFormat=column.info.get("inputFormat"),
            shortcuts=column.info.get("shortcuts"),
            utc=column.info.get("utc"),
            clearable=column.info.get("clearable"),
            embed=column.info.get("embed"),
            timeConstrainst=column.info.get("timeConstrainst"),
        )
        ...
        return TextItem(value=column.info.get("value"))
